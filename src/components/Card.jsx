...
export default function Card({ title, description, price, features }) {
  return (
    <>
      <section className="max-w-xs md:max-w-sm h-max bg-neutral-100 text-center p-8 rounded-xl flex flex-col justify-center items-center gap-4">
        <h2 className="font-bold text-2xl lg:text-3xl">{title}</h2>
        <p className="text-lg">{description}</p>
        <p className="font-bold text-4xl mt-2">{price}</p>

        <ul className="flex flex-col gap-4 text-left mt-2">
